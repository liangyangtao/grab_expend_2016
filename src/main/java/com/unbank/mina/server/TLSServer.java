...
public class TLSServer {
	...
	private static void createAcceptor() {
		try {
			...
			// 设置加密过滤器
			if (Constants.constants.USE_SSL) {
				addSSLSupport(chain);
			}
			// 设置编码过滤器和按行读取数据模式
			...
			keepAliveFilter.setRequestTimeout(Constants.constants.HEART_TIMEOUT);
			// ** 发送频率 *//*
			keepAliveFilter.setRequestInterval(Constants.constants.HEARTBEATRATE);
			...
			// 处理器的代码如下
			// 服务绑定到此端口号
			acceptor.bind(new InetSocketAddress(Constants.constants.SERVERPORT));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
