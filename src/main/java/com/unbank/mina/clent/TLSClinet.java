...
public class TLSClinet {
	...
	private boolean fillConnector() {
		try {
			...
			connector
					.setConnectTimeoutMillis(Constants.constants.CLENT_TIMEOUT);
			// 创建接受数据的过滤器
			...
			keepAliveFilter
					.setRequestTimeout(Constants.constants.HEART_TIMEOUT);
			// ** 发送频率 *//*
			keepAliveFilter
					.setRequestInterval(Constants.constants.HEARTBEATRATE);
			...
			connector.setDefaultRemoteAddress(new InetSocketAddress(
					Constants.constants.SERVERIP,
					Constants.constants.SERVERPORT));
			// 连接到服务器：
